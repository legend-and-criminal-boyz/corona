...
usage() {
        echo -e $w""
        echo -e $w"{"$B$W" CORONA FRAMEWORK "$w"}"
        echo -e $w""
        echo -e $w" usage with commands: corona <"$g"options"$w">"
        echo -e $w" example: "$g"corona run"$w
        echo -e $w""
        echo -e $g" Options:              Descriptions:"
        echo -e $w"---------------------------------------------------"
        echo -e $w" run                   running corona framework"
...
install() {
        echo
        echo -e " \033[41m  "$w$c" Toolname: "$p"corona framework   \033[44m  "$w$c" Author: "$p"iqbalmh18"
        echo -e " \033[42m  "$w$c" Release : "$w"012-07-2020         \033[45m  "$w$c" Github: "$w"saydog.official"
        ...
        echo -e $g"[*]"$w" thank you for installing corona-framework :D"
        echo -e $g"[*]"$w" for information about update saydog framework"
        echo -e $g"[*]"$w" follow me on instagram:- "$g"@labib__mirza"$w
        saydog
}

update() {
        bash ~/corona-framework/update.sh
}
#################### main argv ####################
...
if [ -z "$*" ]; then
        ...
        xdg-open https://github.com/labibhacker/corona-framework/issues 
elif [ $1 == "update" ]; then
        update
        echo -e $g"[+]"$w" successfully update corona framework"
